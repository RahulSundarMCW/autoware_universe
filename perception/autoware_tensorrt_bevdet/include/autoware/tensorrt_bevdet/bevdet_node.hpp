...
private:
  ...
  std::vector<Eigen::Quaternion<float>>
    ego2global_rot_;  ///< Rotation from camera frame to ego frame for each camera
  std::vector<Eigen::Translation3f>
    ego2global_trans_;  ///< Translation from camera frame to ego frame for each camera

  bool transform_dumped_;
